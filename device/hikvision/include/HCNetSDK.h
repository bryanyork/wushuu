...
//大屏控制器（900-950）
#define  NET_ERR_WIN_IDX				        901	// 窗口索引号错误
#define  NET_ERR_WIN_LAYER						902	// 窗口层数错误
#define  NET_ERR_WIN_BLK_NUM			        903	// 窗口的块数错误
...
#define DS_9664N_RX                 199         //NVR:DS_9664N_RX,最后一个X：T/H
...
{
	BYTE byPostCapEnable; //是否启用卡口抓拍，0-否，1-是
	BYTE byDriveLineEnable; //是否启用压车道线抓拍，0-否，1-是
	BYTE byReverseEnable; //是否启用逆行抓拍，0-否，1-是
	BYTE byRedLightEnable; //是否启用闯红灯抓拍，0-否，1-是
	BYTE byVioDirEnable; //是否启用不按导向行驶抓拍，0-否，1-是
	BYTE byIntersectionCongest; //是否启用路口滞留抓拍，0-否，1-是
    BYTE byRes[26];
...
typedef struct tagNET_ITC_LANE_VIDEO_EPOLICE_PARAM
{
	...
	NET_ITC_POLYGON struPlateRecog; //牌识参数
    BYTE  byRes[64];
}NET_ITC_LANE_VIDEO_EPOLICE_PARAM, *LPNET_ITC_LANE_VIDEO_EPOLICE_PARAM;

//违规抓拍参数结构
typedef struct tagNET_ITC_VIOLATION_CAP_PARAM
{
	BYTE bySnapTimes; //抓拍次数(对卡口、逆行、压线、路口滞留有效)，0-3
	BYTE byRes1[3];
	WORD wSnapInterval[MAX_VIDEO_INTERVAL_NUM]; //抓拍间隔时间，单位（毫秒）
	/*只对闯红灯和不按车道行驶车辆抓拍有效*/
	DWORD dwDistance; //电警第2,3张违规图片中车辆的最短位移,单位(像素)
	DWORD dwStopLineDis;  //电警第2张违规图片与停止线的最短距离，单位(像素)
    BYTE byRes[16];
}NET_ITC_VIOLATION_CAP_PARAM, *LPNET_ITC_VIOLATION_CAP_PARAM;
...
typedef struct tagNET_ITC_VIDEO_EPOLICE_PARAM
{
	...
	BYTE byRes1[3];
	NET_ITC_PLATE_RECOG_PARAM struPlateRecog; //牌识参数
	NET_ITC_VIOLATION_DETECT_PARAM struVioDet; //违规检测参数
	...
	NET_ITC_VIOLATION_CAP_PARAM struVioCap; //违规抓拍参数
...
NET_DVR_API BOOL __stdcall NET_DVR_StopInquireSnapDB(LONG lInquireHandle);
...
NET_DVR_API BOOL __stdcall NET_DVR_StopSearchSnapDB(LONG lSearchHandle);
...
NET_DVR_API LONG __stdcall NET_DVR_StopInquireFaceDB(LONG lInquireHandle);
...
NET_DVR_API LONG __stdcall NET_DVR_StopSearchFaceDB(LONG lSearchHandle);
